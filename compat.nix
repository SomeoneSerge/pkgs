# https://github.com/edolstra/flake-compat/blob/0f9255e01c2351cc7d116c072cb317785dd33b33/default.nix
...
let
  ...
  result =
    ...
    then { inputs = null; outputs = callLocklessFlake rootSrc; }
    else if lockFile.version == 4
    then { inputs = parseInputs rootSrc lockFile.inputs; outputs = callFlake4 rootSrc (lockFile.inputs); }
    else if lockFile.version >= 5 && lockFile.version <= 7
    then { inputs = allNodes; outputs = allNodes.${lockFile.root}; }
